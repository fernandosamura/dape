...
module.exports = {
  up: async (queryInterface: QueryInterface) => {
    await queryInterface.sequelize.query(`
      CREATE TABLE IF NOT EXISTS daple_shield_config (
        ...
        degraded_mode_enabled    BOOLEAN NOT NULL DEFAULT TRUE,
      ...
      CREATE INDEX IF NOT EXISTS idx_shield_audit_company_date
        ON daple_shield_audit_log (company_id, created_at DESC);
      CREATE INDEX IF NOT EXISTS idx_shield_audit_whatsapp_date
        ON daple_shield_audit_log (whatsapp_id, created_at DESC);
      ...
      CREATE TABLE IF NOT EXISTS daple_shield_counters (
        ...
        updated_at   TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
        UNIQUE (whatsapp_id, window_type, window_key)
      );

      CREATE TABLE IF NOT EXISTS daple_shield_quarantine (
        id               SERIAL PRIMARY KEY,
        whatsapp_id      INTEGER NOT NULL UNIQUE,
        company_id       INTEGER NOT NULL,
  ...
  down: async (queryInterface: QueryInterface) => {
    await queryInterface.sequelize.query(`
      DROP TABLE IF EXISTS daple_shield_quarantine;
      DROP TABLE IF EXISTS daple_shield_counters;
      DROP TABLE IF EXISTS daple_shield_audit_log;
      DROP TABLE IF EXISTS daple_shield_config;
    `);
  },
};
