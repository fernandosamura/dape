...
if [[ -z "$REACT_APP_TAB_NAME" ]]; then
    echo -e "${YELLOW}⚠️  REACT_APP_TAB_NAME não definida, usando valor padrão: DAPE${NC}"
    TAB_NAME="DAPE"
else
...
# Substitui todas as ocorrências de "DAPE" pelo novo nome
sed -i "s/DAPE/$TAB_NAME/g" public/index.html
...
if [[ -f "public/manifest.json" ]]; then
    sed -i "s/DAPE/$TAB_NAME/g" public/manifest.json
